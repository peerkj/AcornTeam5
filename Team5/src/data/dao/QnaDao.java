...
public class QnaDao {
   ...
   //�궡媛� �벖 吏덈Ц 蹂닿린
   public List<QnaDto> getClientQnaList(String id, int start, int end){
      List<QnaDto> list= new ArrayList<QnaDto>();
      Connection conn=null;
      PreparedStatement pstmt=null;
      ResultSet rs=null;
      String sql="select * from (select qna.*, rownum snum from qna where id=? order by qwriteday desc) where snum>=? and snum<=?";
      conn=db.getConnection();
      try {
         pstmt=conn.prepareStatement(sql);
   ...
   public int useCount(String id) {
      ...
      ResultSet rs= null;
      int count = 0;
      String sql="select count(*) count from qna where id=?";
